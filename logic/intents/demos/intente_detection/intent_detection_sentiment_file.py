...
class IntentDetectionSentimentFile:
    """Detects user intent to retrieve sentiment files (Q10 or K10 curated reports)."""

    Q10_base_folder = "Q10_sentiment_summary_report"
    K10_base_folder = "K10_sentiment_summary_report"
    ...
    def detect(self, question: str) -> Optional[str]:
        ...
        # --- SYMBOL detection (MUCH more strict now)
        sym_match = re.search(r'\b(?:DE|DEL|DE LA|SYMBOL)\s+([A-Z]{1,6})\b', text)
        if not sym_match:
            # fallback: last uppercase word, often the ticker
