...
for root,dirs,files in os.walk(ROOT):
    ...
    # load triple
    chunks=[x.strip() for x in open(c,encoding="utf-8").read().split("\n\n") if x.strip()]
    meta=json.load(open(m,encoding="utf-8"))
    emb=np.load(e).astype("float32")

    # strict triple validation
    if len(chunks)!=len(meta) or emb.shape[0]!=len(chunks):
        print("WARNING: SKIPPED BROKEN FOLDER:", root)
        continue
...
json.dump(CONFIG, open(os.path.join(OUT,"text-embedding-3-large.json"),"w"), indent=2)
